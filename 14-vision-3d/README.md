...
| 目录编号 | Notebook | 核心问题 |
| --- | --- | --- |
| 01 | [从零实现 Masked Autoencoder](./01-masked-autoencoder-from-scratch-pytorch.ipynb) | patch 双射、逐样本确定性 masking、visible-only encoder、restore index、mask token decoder 与 masked-only loss如何实现？ |
| 02 | [从零实现 SAM 风格可提示分割](./02-sam-promptable-segmentation-from-scratch-pytorch.ipynb) | 正/负/填充点、box prompt、双向 cross-attention、多 mask、IoU head 与最佳候选选择如何串联？ |
| 03 | [从零实现教学版 3D Gaussian Splatting](./03-3d-gaussian-splatting-from-scratch-pytorch.ipynb) | 相机投影、Jacobian covariance、椭圆权重、深度排序、front-to-back 合成与可微优化如何实现？ |
