...
| 219 | [RAG Contextual Retrieval 与来源版本](./219-rag-contextual-retrieval-provenance-versioning-from-scratch.ipynb) | 怎样将文档级 context 加入检索表示，同时保留 raw evidence、ACL、生成器版本和失效门禁？ |
| 220 | [RAPTOR 分层摘要检索与来源链](./220-raptor-hierarchical-summary-retrieval-provenance-from-scratch.ipynb) | 怎样递归聚类/摘要，按层检索后展开 leaf evidence，并处理 summary 的版本失效与错误传播？ |
| 221 | [长上下文 Needle 位置鲁棒性评测](./221-llm-long-context-needle-position-robustness-evaluation-from-scratch.ipynb) | 怎样控制长度、位置、干扰、解析与无答案对照，避免用单一 needle 误判真实长文理解？ |
| 222 | [Agent Loop 停滞检测与停止策略](./222-agent-loop-stagnation-fingerprint-budget-stop-policy-from-scratch.ipynb) | 怎样基于 action/state fingerprint、进展、预算和副作用语义识别循环，并避免误杀正常轮询？ |
| 223 | [Text-to-SQL Schema Linking 与 Policy Gate](./223-text-to-sql-schema-linking-policy-gate-from-scratch.ipynb) | 怎样将模型候选限制为 schema-aware AST，执行表/列/租户/参数/成本门禁并保存结果证据？ |
| 224 | [GraphRAG Local/Global Search 与来源](./224-graphrag-local-global-community-provenance-from-scratch.ipynb) | 怎样按实体邻域或社区报告选择检索路径，并将 edge/summary 展开为可核验原文？ |
| 225 | [Temporal RAG 有效时间、冲突与引用](./225-temporal-rag-valid-time-conflict-citation-from-scratch.ipynb) | 怎样按 valid-time 检索、拒绝同一时点的冲突事实，并给答案附上生效区间和来源？ |
| 226 | [Agent 指令层级冲突与来源控制](./226-agent-instruction-hierarchy-conflict-provenance-from-scratch.ipynb) | 怎样解析 system/developer/user/external 的冲突，处理同级澄清、外部注入与副作用授权？ |
| 227 | [Web Research Agent 证据账本与引用](./227-web-research-agent-claim-evidence-citation-ledger-from-scratch.ipynb) | 怎样规划搜索、过滤来源、建立 claim-evidence ledger、处理冲突/时效，并只渲染可核验主张？ |
...
| 235 | [Medusa 多解码头与树形验证](./235-medusa-multi-head-tree-verification-from-scratch.ipynb) | 怎样用多个未来 token head 构造候选树、tree attention 批量验证，并只提交 target 接受路径与 KV？ |
